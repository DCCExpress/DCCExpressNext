...
export function getCurrentCommandCenter() {
    return commandCenter;
}
export function initializeCommandCenter(conf) {
    if (commandCenter) {
        commandCenter
            .stop()
            .then(() => {
    ...
    switch (conf?.type) {
        ...
        case "z21":
            ...
            commandCenter =
                new Z21CommandCenter("Z21", conf.z21.host, conf.z21.port, message => {
                    broadcast?.(message);
                });
            commandCenter
                .start()
...
export function getLogicalTurnoutState(address) {
...
export function registerCommandCenterConfigLoadedCallback() {
