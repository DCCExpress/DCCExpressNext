...
export const fastClockRoutes = Router();
fastClockRoutes.get("/", (_req, res) => {
    res.json(fastClockRuntimeStore.getSnapshot());
});
fastClockRoutes.post("/run", (_req, res) => {
    res.json(fastClockRuntimeStore.run());
});
fastClockRoutes.post("/pause", (_req, res) => {
    res.json(fastClockRuntimeStore.pause());
});
fastClockRoutes.post("/reset", (_req, res) => {
    res.json(fastClockRuntimeStore.reset());
});
fastClockRoutes.post("/speed", (req, res) => {
    ...
    if (!Number.isFinite(speed) ||
        ...
        res.status(400).json({
            ok: false,
            error: "A fast clock sebességszorzója legalább 1 kell legyen.",
        });
        return;
    }
    res.json(fastClockRuntimeStore.setSpeed(speed));
});
