...
layoutRoutes.get("/topology-summary", (_req, res) => {
    ...
    res.json({
        ready: true,
        physicalTracks: topology.getPhysicalTrackElements().length,
        turnouts: topology.getTurnouts().length,
        blocks: topology.getBlocks().length,
        sensors: topology.getSensors().length,
        signals: topology.getSignals().length,
        directionElements: topology.getDirectionElements().length,
    });
});
layoutRoutes.get("/route-graph-summary", (_req, res) => {
    ...
    const blocks = graph.nodes.flatMap(node => node.blocks);
    res.json({
        ready: true,
        ...
        runnableBlockRoutes: graph.getRunnableBlockRoutes().length,
        runnableBlockTransitions: graph.getRunnableBlockTransitions().length,
    });
});
// layoutRoutes.get("/route-graph-summary", (_req, res) => {
//   const graph = routeGraphRuntimeStore.getGraph();
...
layoutRoutes.get("/route-test", (req, res) => {
    ...
    const fromBlockName = typeof req.query.from === "string"
        ? req.query.from
        : "A1";
    const toBlockName = typeof req.query.to === "string"
        ? req.query.to
        : "C3";
    const fromBlock = graph.findBlockByName(fromBlockName);
    const toBlock = graph.findBlockByName(toBlockName);
    ...
    const toNode = toBlock
        ...
        : null;
    const nodeRoute = fromNode && toNode
        ? graph.findRoute(fromNode.name, toNode.name)
        : null;
    const blockRoute = graph.findRouteBetweenBlockNames(fromBlockName, toBlockName);
    res.json({
        ready: true,
        request: {
            fromBlockName,
...
layoutRoutes.put("/runtime", async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            message: "Nem sikerült frissíteni a szerveroldali runtime layoutot.",
        });
    }
});
