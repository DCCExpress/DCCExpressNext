...
export class TrackSignalElementView extends TrackElement implements ITrackSignalElement {
    ...
    static fromJSON(data: ITrackSignalElement) : TrackSignalElementView {
        const e = new TrackSignalElementView(data.x, data.y);
        e.id = data.id;
        e.name = data.name;
    ...
    override clone(): TrackSignalElementView {
        const copy = new TrackSignalElementView(this.x, this.y);
        copy.id = generateId();
