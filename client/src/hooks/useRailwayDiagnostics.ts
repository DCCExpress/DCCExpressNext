...
export type DiagnosticSensorItem = { address: number; on: boolean };
export type DiagnosticTurnoutItem = { address: number; closed: boolean };
...
function addAccessorySource(
  ...
  target[address] = [...(target[address] ?? []), source];
}
...
function addAddressRange(
  ...
  for (let offset = 0; offset < Math.max(0, Math.floor(length)); offset++) {
    addAccessorySource(target, startAddress + offset, source);
  }
}

function isSignalElementType(type: unknown): boolean {
  return type === ELEMENT_TYPES.TRACK_SIGNAL2 || type === ELEMENT_TYPES.TRACK_SIGNAL3 || type === ELEMENT_TYPES.TRACK_SIGNAL4;
}

function isTurnoutElementType(type: unknown): boolean {
  return type === ELEMENT_TYPES.TRACK_TURNOUT_LEFT ||
...
function isButtonAccessoryElementType(type: unknown): boolean {
  return type === ELEMENT_TYPES.BUTTON ||
...
function readConfiguredAddresses(): ConfiguredRuntimeAddresses {
  ...
  for (const element of layoutStore.getElements()) {
    ...
    if (item.type === ELEMENT_TYPES.TRACK_SENSOR && typeof item.address === "number") {
      sensors.push(item.address);
    }

    if (isTurnoutElementType(item.type) && typeof item.turnoutAddress === "number") {
      turnouts.push(item.turnoutAddress);
      addAccessorySource(accessories, item.turnoutAddress, createSource(item));
    }

    if (item.type === ELEMENT_TYPES.TRACK_LEVEL_CROSSING && typeof item.basicAccessoryAddress === "number") {
      addAccessorySource(accessories, item.basicAccessoryAddress, createSource(item));
    }

    if (isButtonAccessoryElementType(item.type) && typeof item.address === "number") {
      addAccessorySource(accessories, item.address, createSource(item));
    }

    if (isSignalElementType(item.type) && typeof item.address === "number") {
      addAddressRange(
        accessories,
        item.address,
        typeof item.addressLength === "number" ? item.addressLength : 1,
        createSource(item)
      );
    }
  }
...
function mergeConfiguredSensors(
  ...
  for (const address of configured) result.set(address, { address, on: false });
  for (const item of Object.values(runtime)) result.set(item.address, item);
...
function mergeConfiguredTurnouts(
  ...
  for (const address of configured) result.set(address, { address, closed: false });
  for (const item of Object.values(runtime)) result.set(item.address, item);
  return sortByAddress(Array.from(result.values()));
}
...
export function useRailwayDiagnostics() {
  const [configuredAddresses, setConfiguredAddresses] = useState<ConfiguredRuntimeAddresses>(() => readConfiguredAddresses());
  ...
  useEffect(() => layoutStore.subscribe(() => {
  ...
  useEffect(() => {
    const unsubscribeSensor = wsClient.on("sensorChanged", (data: SensorChangedPayload) => {
      setSensors(current => ({ ...current, [data.address]: { address: data.address, on: data.on } }));
    });

    const unsubscribeTurnout = wsClient.on("turnoutChanged", (data: TurnoutChangedPayload) => {
      setTurnouts(current => ({ ...current, [data.address]: { address: data.address, closed: data.closed } }));
    });
  ...
  return {
    sensors: useMemo(() => mergeConfiguredSensors(configuredAddresses.sensors, sensors), [configuredAddresses.sensors, sensors]),
    turnouts: useMemo(() => mergeConfiguredTurnouts(configuredAddresses.turnouts, turnouts), [configuredAddresses.turnouts, turnouts]),
    accessories: useMemo(() => mergeConfiguredAccessories(configuredAddresses.accessories, accessories), [configuredAddresses.accessories, accessories]),
  };
}
