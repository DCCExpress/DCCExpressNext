...
function buildGeneratedScript(groups: SignalRuleGroup[]): string {
  ...
  for (const address of turnoutAddresses) {
    lines.push(
      `  const t${address}Closed = getTurnoutState(${address});`
    );
  }
  ...
  for (const group of groups) {
    ...
    group.rules.forEach((rule, ruleIndex) => {
      ...
      const expression = rule.conditions.length === 0
        ? "true"
        : rule.conditions
          .map(condition => {
      ...
      lines.push(
        `    await ${aspectToMethod(rule.aspect)}(${group.signalAddress});`
      );
      lines.push("  }");
    });
    ...
    lines.push("  else {");
    lines.push(
      `    await ${aspectToMethod(group.defaultAspect)}(${group.signalAddress});`
    );
    lines.push("  }");
    lines.push("");
  }
...
export default function SignalLogicDialog({
  ...
  const [groups, setGroups] = useState<SignalRuleGroup[]>(() => {
    ...
    return firstSignalAddress > 0
      ? [createRule(firstSignalAddress)]
      : [];
  });
  ...
  const addSignalRuleGroup = (): void => {
    const usedAddresses = new Set(
      groups.map(group => group.signalAddress)
    );
    ...
    const signalAddress = Number(
      nextSignal?.value ?? signalOptions[0]?.value ?? 0
    );
            ...
            <Box flex={1}>
              {!selectedGroup ? (
                <Card withBorder p="lg">
                  <Text c="dimmed">
                    Select or add a signal rule group.
                  </Text>
                </Card>
              ) : (
                <Stack gap="md">
                        ...
                        {rule.conditions.map((condition, conditionIndex) => (
                          <Group key={condition.id} align="flex-end">
                            ...
                            <Select
                              ...
                              data={turnoutOptions}
                              value={condition.turnoutAddress > 0
