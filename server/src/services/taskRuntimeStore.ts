...
import {
  parseTrainTasksDocument,
  serializeTrainTasksDocument,
} from "./tasks/taskPersistence.js";

import {
  taskPersistenceStore,
} from "./tasks/taskPersistenceStore.js";
...
class TaskRuntimeStore {
  ...
  async initialize(): Promise<void> {
    ...
    const document = await taskPersistenceStore.load();
    const parseResult = parseTrainTasksDocument(document);

    this.tasks = parseResult.tasks;
  ...
  private createActionResult(ok: boolean, error?: string): TaskManagerActionResult {
    return {
      ok,
      ...(error ? { error } : {}),
  ...
  async saveTasks(): Promise<TaskManagerActionResult> {
    ...
    await taskPersistenceStore.save(
      serializeTrainTasksDocument(this.tasks)
    );
  ...
  hasActiveTasks(): boolean {
    return this.tasks.some(task => task.status === "running" || task.status === "paused" || task.status === "finishing");
  }
  ...
  async startTask(taskIdOrName: string): Promise<TaskManagerActionResult> {
    ...
    const route = routeGraphRuntimeStore.findRouteBetweenBlockIds(task.fromBlockId, task.toBlockId);
    ...
    if (!locoAssigned && !task.runtime.locoAddress) {
      ...
      task.status = "queued";
      this.broadcast?.({ type: "taskWaitingForLoco", data: { taskId: task.id, taskName: task.name } });
      this.broadcastSnapshot();
  ...
  private async runSimulationStep(taskId: string): Promise<void> {
    ...
    if (progress.completed) {
      await this.finishTask(task.id);
      this.broadcast?.({ type: "taskCompleted", data: { taskId: task.id, taskName: task.name } });
      return;
    }
