...
export type TaskRuntimeCommands = {
  setLoco(
    address: number,
    speed: number,
...
class ServerTaskSession {
  ...
  private state: TaskStateDto;
  private readonly listeners =
    new Set<TaskStateListener>();

  private index = 0;
  private locoAddress = 0;
  private stopRequested = false;
  private paused = false;
  private pauseResolver: (() => void) | null = null;

  constructor(
    private readonly definition: TaskDefinitionDto,
    private readonly commands: TaskRuntimeCommands,
    private readonly broadcast: BroadcastFn | null
  ) {
    this.state = {
      id: this.id,
  ...
  subscribe(listener: TaskStateListener) {
  ...
  private async executeStep(step: TaskStepDto): Promise<void> {
    switch (step.type) {
      case "setLoco": {
      ...
      case "setFunction": {
        ...
        if (!this.locoAddress) {
          throw new Error(
            "setFunction requires a prior setLoco step."
          );
        }
        ...
        if (!ok) {
          throw new Error(
            `Could not set F${fn} for loco ${this.locoAddress}.`
          );
        }
      ...
      case "waitForSensor": {
        ...
        await this.waitUntil(async () => {
          const state =
            await this.commands.getSensorState(address);
      ...
      case "ifClosed": {
        const address = numberData(step, "address");
        const state =
          this.commands.getTurnoutState(address);
      ...
      case "setAccessory": {
        ...
        if (!ok) {
          throw new Error(
            `Could not set accessory/output #${address}.`
          );
        }
      ...
      case "ifAccessoryIsOn": {
        const address = numberData(step, "address");
        const state =
          this.commands.getAccessoryState(address);
      ...
      case "setRoute": {
        ...
        throw new Error(
          `setRoute("${routeName}") is not migrated yet. Use the server route reservation flow instead.`
        );
      }

      case "ifFree": {
        throw new Error(
          "ifFree is not migrated yet because the current server route model uses reservations."
        );
      }
      ...
      default: {
        ...
        throw new Error(
          `Unsupported task step: ${String(neverStep)}`
        );
      }
    }
  }

  private async setTurnout(
    address: number,
    closed: boolean
  ): Promise<void> {
    await this.check();
  ...
  private async setLocoSpeed(
    speed: number,
    direction: "forward" | "reverse"
  ): Promise<void> {
    if (!this.locoAddress) {
      throw new Error(
        "Loco motion requires a prior setLoco step."
      );
    }

    const ok =
      await this.commands.setLoco(
        this.locoAddress,
        speed,
        direction
      );
    ...
    if (!ok) {
      throw new Error(
        `Could not set loco ${this.locoAddress}.`
      );
    }
  }
  ...
  private isSignalAspect(
    ...
    for (
      ...
      const current =
        this.commands.getAccessoryState(
          signal.address + i
        );
  ...
  private getSignal(address: number) {
    const topology =
      railwayTopologyStore.getTopology();
  ...
  private gotoLabel(label: string): void {
    const targetIndex =
      this.definition.steps.findIndex(step => {
        return (
          step.type === "label" &&
    ...
    if (targetIndex < 0) {
      throw new Error(
        `Could not find task label "${label}".`
      );
    }
  ...
  private gotoNextEndOrElse(): void {
    const target =
      this.findMatchingBranchTarget(
        this.index,
        true
      );
    ...
    if (target < 0) {
      throw new Error(
        "Could not find matching else/endIf."
      );
    }
  ...
  private findMatchingBranchTarget(
    fromIndex: number,
    includeElse: boolean
  ): number {
    let depth = 0;
    ...
    for (
      let i = fromIndex + 1;
      i < this.definition.steps.length;
      i++
    ) {
...
function numberData(
  ...
  if (typeof value !== "number") {
    throw new Error(
      `${step.type}.${key} must be a number.`
    );
  }
...
function booleanData(
  ...
  if (typeof value !== "boolean") {
    throw new Error(
      `${step.type}.${key} must be a boolean.`
    );
  }
...
function stringData(
  ...
  if (typeof value !== "string") {
    throw new Error(
      `${step.type}.${key} must be a string.`
    );
  }
...
function numberArrayData(
  ...
  if (
    ...
    throw new Error(
      `${step.type}.${key} must be a number array.`
    );
  }
...
class TaskRuntimeStore {
  ...
  private readonly sessions =
    new Map<string, ServerTaskSession>();

  private readonly filePath =
    path.resolve(dataDir, "tasks.json");

  configure(params: {
    commands: TaskRuntimeCommands;
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      const parsed =
        JSON.parse(raw) as Partial<TaskDocumentDto>;
  ...
  async startTask(
    ...
    if (!this.commands) {
      throw new Error(
        "Task runtime commands are not configured."
      );
    }
    ...
    if (!definition) {
      throw new Error(
        `Task not found: ${taskIdOrName}`
      );
    }

    const current =
      this.sessions.get(definition.id);
    ...
    if (
      ...
      throw new Error(
        `Task "${definition.name}" is already active.`
      );
    }

    const session =
      new ServerTaskSession(
        definition,
        this.commands,
        this.broadcast
      );
  ...
  stopTask(taskIdOrName: string): void {
    const definition =
      this.findTask(taskIdOrName);
  ...
  pauseTask(taskIdOrName: string): void {
    const definition =
      this.findTask(taskIdOrName);
  ...
  resumeTask(taskIdOrName: string): void {
    const definition =
      this.findTask(taskIdOrName);
  ...
  async autoStartEnabledTasks(): Promise<void> {
    await this.initialize();
    ...
    for (const task of this.document.tasks) {
      ...
      try {
        ...
        console.error(
          `[TaskRuntimeStore] Could not auto-start task "${task.name}":`,
          error
        );
      }
    }
...
function normalizeTaskDocument(
  ...
  return {
    tasks: tasks
      ...
      .map((task, index) => {
        const raw =
          task as Partial<TaskDefinitionDto>;
          ...
          steps:
            Array.isArray(raw.steps)
              ? raw.steps
                  ...
                  .map(step => {
                    const rawStep =
                      step as Partial<TaskStepDto>;

                    return {
                      type: String(rawStep.type ?? "break") as TaskStepType,
