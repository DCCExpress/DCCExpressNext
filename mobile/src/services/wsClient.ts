...
type MessageListener = (message: TypedServerWsMessage) => void;

type TypedMessageListener<TType extends ServerWsMessageType> = (
...
class WsClient {
  ...
  private typedListeners = new Map<string, Set<AnyTypedMessageListener>>();
  ...
  connect(url?: string): void {
    ...
    this.socket.onmessage = event => {
      try {
        const message = JSON.parse(event.data) as TypedServerWsMessage;
  ...
  on<TType extends ServerWsMessageType>(
    type: TType,
    listener: TypedMessageListener<TType>
  ): () => void {
    const listeners =
      this.typedListeners.get(type) ?? new Set<AnyTypedMessageListener>();
